/*
  ...
  1. Changes
    ...
    - Add more robust error handling to prevent profile creation failures
    - Ensure proper name extraction from Google profile data
    - Fix RLS policies to allow public access for profile operations

  2. Security
    - Maintain proper access control while fixing authentication issues
    - Ensure users can only modify their own profiles
    - Allow reading other profiles for display purposes
*/
...
DROP POLICY IF EXISTS "Allow authenticated users to read all user profiles" ON public.user_profiles;
