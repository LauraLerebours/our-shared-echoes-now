/*
  ...
  1. Problem
    - Google OAuth users are not getting proper profiles created
    - The trigger function is not properly handling Google metadata
    - Users are getting redirected back to login page after Google auth

  2. Solution
    - Create a more robust handle_new_user() function
    - Add a trigger for when user metadata is updated (crucial for OAuth flows)
    - Add better error handling to prevent trigger failures
    - Create a function to manually create missing profiles

  3. Changes
    - Update handle_new_user() to extract names from Google metadata
    - Add trigger for metadata updates
